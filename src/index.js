...
app.get("/:id", async (req, res) => {
  ...
  switch (id) {
    case "1":
      sensorServerURL = sensorURLs.Keith
      break;
    case "2":
      sensorServerURL = sensorURLs.Mark
      break;
    case "3":
      sensorServerURL = sensorURLs.Jayden
      break;
    default:
  ...
  try {
    console.log("attempting to reach arduino at " + sensorServerURL)
    const sensorResponse = await axios.get(sensorServerURL, { timeout: 9000 })
...
const emailUser = (user, sensor) => {
  const mailOptions = {
    ...
    <div style="font-size: 1.5em; font-weught: 700; color: #163d5f; background-image: url('cid:unique@kreata.ee'); background-position: bottom; background-size: 75%; padding: 20px;">
      ...
      <br>
      <a style="font-size: 0.5em" href="${BACKEND_URL}/alerts/${user.users_id}/remove/${user.sensors_id}">Stop receiving alerts for this sensor</a>
      <br>
      <a style="font-size: 0.5em" href="${BACKEND_URL}/alerts/${user.users_id}/remove/">Stop receiving all alerts</a>
...
const newSensorEmail = (sensor, sensorIds) => {
  const mailOptions = {
    ...
    <div style="font-size: 1.5em; font-weught: 700; color: #163d5f; background-image: url('cid:unique@kreata.ee'); background-position: bottom; background-size: 75%; padding: 20px;">
      ...
      href="${BACKEND_URL}/sensors/${sensorIds.users_id}/update/${sensorIds.sensors_id}/${sensor.name}/null/${sensor.latitude}/${sensor.longitude}">
        Confirm
      </a>
